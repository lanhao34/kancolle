...
POS_CHANGE_SHIP	=	[(410+340*x,217+113*y) for y in xrange(3) for x in xrange(2)]
...
def match(screen, filename, threshold = 300000, return_pos = False):
...
class AutoClick:
	...
	def init(self):
		tmp,pos=match(screenshot(),"setting.bmp", threshold = 500000, return_pos = True)
		while not tmp:
			sleep(3)
			tmp,pos=match(screenshot(),"setting.bmp", threshold = 500000,return_pos = True)
		
		self.stype=dict((key,[ship['api_sortno'] for ship in group]) for key,group in groupby(sorted(self.server.data['/kcsapi/api_start2']['api_data']['api_mst_ship'],key=itemgetter('api_stype')),key=itemgetter('api_stype')))
		
		self.ships_by_sortno=dict((ship['api_sortno'],ship) for ship in self.server.data['/kcsapi/api_start2']['api_data']['api_mst_ship'])
	...
	def launch(self):
		...
		mouse.click(*POS_GO)
		sleep(1)
		screen=self.screen.shot(region=get_region([self.box[2],self.box[0],800,condition[self.mission]]))
		...
		while 1:
			...
			while flag==0 and not self.flag:
				...
				if formation and check_night and match(screen,"compass.bmp"):
					...
					mouse.click(*POS_FORMATION[self.mission])
					formation=False
					sleep(20)
	...
	def update_data(self):
		...
		empty_dock = [i for i,dock in enumerate(port_api_data['api_ndock']) if dock['api_state']==0]
		ship_in_dock=set(dock['api_ship_id'] for dock in port_api_data['api_ndock'])
		...
		for i,ship_id in enumerate(self.ship_team1):
			...
			if ship_id in ship_in_dock:
				...
				need_replace.add(i)
				self.max_repair_time=max(self.max_repair_time,now_time+(33-ship['api_cond'])/3*180+180)
		...
		self.max_repair_time=min(self.max_repair_time,self.min_expdition_time)
	...
	def main(self):
		# print self.server.data['/kcsapi/api_port/port']['api_data']['api_deck_port'][0]['api_ship']
		
		# self.tid=win32api.GetCurrentThreadId()
		# sleep(1)
		# print self.tid
		self.box=self.init()
		self.screen = ScreenShoter(region=get_region([self.box[2], self.box[0], 800, 480]))
